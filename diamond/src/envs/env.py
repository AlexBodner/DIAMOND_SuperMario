...
class DoneOnLifeLoss(gymnasium.Wrapper):
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[str, Any]]:
        ...
        # Calculate life loss from the difference in lives
        curr_lives = info.get("lifes", None)
        if self._prev_lives is not None and curr_lives is not None:
            life_loss = curr_lives < self._prev_lives
            if isinstance(life_loss, (bool, np.bool_)):
                life_loss = np.array([life_loss])  # Convert single boolean to array
            
            if life_loss.any():
                end[life_loss] = True
                info["final_observation"] = obs
        
        self._prev_lives = curr_lives
        return obs, rew, end, trunc, info
    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        self._prev_lives = info.get("lifes", None)
        return obs, info
